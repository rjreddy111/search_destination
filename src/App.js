...
class App extends Component {
  state = {searchInput: '', InitalDestinationList: destinationsList}
  ...
  render() {
    const {searchInput, InitalDestinationList} = this.state
    const searchResults = InitalDestinationList.filter(eachList =>
          ...
          <div className="search-container">
            ...
            <ul className="unorderedList">
              {searchResults.map(eachList => (
                <DestinationItem eachListDetails={eachList} key={eachList.id} />
              ))}
            </ul>
          </div>
        </div>
